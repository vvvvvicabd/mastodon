...
class Api::V1::EncryptedMessagesController < Api::BaseController
  ...
  def next_path
    if records_continue?
      api_v1_encrypted_messages_url pagination_params(max_id: pagination_max_id)
    end
  end

  def prev_path
    unless @encrypted_messages.empty?
      api_v1_encrypted_messages_url pagination_params(min_id: pagination_since_id)
    end
  end
